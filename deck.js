...
(function () {
  ...
  function initParallax() {
    ...
    slides.forEach(function (slide) {
      ...
      slide._stage = slide.querySelector('.stage');
      slide._dim = 0;
    ...
    /* Сглаженная позиция скрола. Сами секции липкие и едут нативно, один в один
       с колесом, а вот всё, что ведёт этот цикл — параллакс, затемнение шторки,
       появление — тянется за ней с задержкой. Без этого один щелчок колеса
       мгновенно перебрасывает эффекты на сотню пикселей, и шторка выглядит
       дёрганой. */
    ...
    function frame() {
      ...
      sy += (raw - sy) * 0.085;
      if (Math.abs(raw - sy) < 0.5) sy = raw;   /* добираем хвост, чтобы не дрожало */
      var y = sy;
      ...
      for (var s = 0; s < slides.length; s++) {
        ...
        for (var j = 0; j < items.length; j++) {
          ...
          /* Текст выходит, когда шторка уже дошла до верха, плитки — сразу
             следом. Дальше скрол ни при чём: у каждой плитки своя задержка
             старта и своя скорость доезда, поэтому коллаж набирается не
             монолитом, но и не пропадает, если пролистать быстро. */
          var porog = n.classList.contains('tile') ? -vh * 0.02 : vh * 0.03;
          if (flowTop < porog) {
            show(n);
            n._shown = true;
        ...
        /* Уход под шторку: секцию, которую накрывают, затемняем — иначе край
           шторки не читается, кремовое наезжает на кремовое. Включается не
           сразу, а когда шторка прошла 70 % экрана, дальше 0,7…1 разгоняется
           до предела. */
        if (slide._stage) {
          /* Накрывать начинают не сразу: следующая секция входит в окно, когда
             у этой остаётся ровно экран. Отсчитываем от этого момента, иначе
             на длинных разделах затемнение доходило бы до предела задолго
             до того, как шторка вообще появится. */
          var over = -flowTop - (slide._h - vh);
          var cover = over > 0 ? Math.min(1, over / vh) : 0;
          var dim = cover > 0.7 ? (cover - 0.7) / 0.3 : 0;
          dim = dim * dim * (3 - 2 * dim);      /* сглаживаем вход и выход рампы */
          if (dim !== slide._dim) {
            slide._dim = dim;
            slide._stage.style.setProperty('--dim', (dim * 0.6).toFixed(3));
          }
        }

        /* большой заголовок уходит обратно под маску, когда экран поехал вверх */
        if (slide._outs.length) {
          /* Заголовок стоит первые 20 % экрана, потом уходит под маску. Раздел
             начинает приходить на 40 % — то есть между началом ухода и шторкой
             остаётся ещё 20 % экрана, чтобы буквы успели уехать. */
          var gone = flowTop < -vh * 0.2;
          if (gone !== slide._gone) {
            slide._gone = gone;
        ...
        for (var i = 0; i < own.length; i++) {
          ...
          /* выезд снизу: чем ближе слой, тем длиннее его путь — так плитки
             приходят не одной плитой, а с разбросом по глубине */
          var dy = (slide._still ? 0 : -p * d * vh * 0.55 + my * d * 40)
                 + settle * (140 + d * 520);
